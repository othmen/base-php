...
for version in "${versions[@]}"; do
    ...
    sed -i -E "s/\`$majorVersion.$minorVersion.[0-9]+-debug\`/\`$fullVersion-debug\`/" README.md
    sed -i -E "s/\`$majorVersion.$minorVersion.[0-9]+\`/\`$fullVersion\`/" README.md
    sed -i -E "s/(PHP$majorVersion$minorVersion)=.*/\1=$fullVersion/" .travis.yml
	...
	for suite in alpine3.7; do
		...
		for variant in fpm; do
			...
			' "$version/$suite/$variant/Dockerfile.wodby"
			...
			if [ "$majorVersion" = '5' ] || [ "$majorVersion" = '7' -a "$minorVersion" -lt '2' ] || [ "$suite" = 'jessie' ]; then
				...
				sed -ri '/argon2/d' "$version/$suite/$variant/Dockerfile.wodby"
			...
			if [ "$majorVersion" = '5' ] || [ "$majorVersion" = '7' -a "$minorVersion" -lt '2' ]; then
				...
				sed -ri '/sodium/d' "$version/$suite/$variant/Dockerfile.wodby"
			fi
			if [ "$majorVersion" = '5' -a "$suite" = 'stretch' ]; then
				# php 5 still needs older ssl
				sed -ri 's/libssl-dev/libssl1.0-dev/g' "$version/$suite/$variant/Dockerfile.wodby"
			fi
			...
			awk '
				...
				NF == 0 { ++blank }
				blank < 2 { print }
			' "$version/$suite/$variant/Dockerfile.wodby" > "$version/$suite/$variant/Dockerfile.new"
			mv "$version/$suite/$variant/Dockerfile.new" "$version/$suite/$variant/Dockerfile.wodby"
			...
			sed -ri \
				...
				"$version/$suite/$variant/Dockerfile.wodby"
			dockerfiles+=( "$version/$suite/$variant/Dockerfile.wodby" )
		done
	done
